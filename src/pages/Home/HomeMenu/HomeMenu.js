                    ...
                    {heThongRap.lstCumRap?.slice(0, 10).map((cumRap, index) => {
                        return <TabPane tab={
                            <div className='w-[100px] sm:w-[150px] md:w-[200px] lg:w-[270px] xl:w-[350px]' style={{ display: 'flex', alignItems: 'center' }}>
                                <img src={cumRap.hinhAnh} style={{ width: '50px', height: '50px' }} alt={cumRap.tenCumRap} />
                                ...
                                return <Fragment key={index} >
                                    <div className='mb-2'>
                                        <div style={{ display: 'flex' }}>
                                            <img style={{ width: '60px', height: '80px' }} src={phim.hinhAnh} alt={phim.tenPhim} onError={(e) => (e.target.onerror = null, e.target.src = 'https://picsum.photos/68/80')} />
                                            <div className='ml-1 md:ml-[10px!important] text-wrap break-all'>
                                                <h1 className='text-xl md:text-2xl text-white'>{phim.tenPhim}</h1>
                                                <p className='text-md md:text-lg text-white mb-0'>{cumRap.diaChi}</p>
                                            </div>
                                        </div>
                                        <div style={{ display: 'flex', justifyItems: 'center', alignItems: 'center' }}>
                                            <img style={{ width: '60px', height: '60px' }} src="https://cdn-icons-png.flaticon.com/512/1666/1666756.png" alt="" className='mt-1' />
                                            <div className='grid grid-cols-5 gap-1'>
                                                {phim.lstLichChieuTheoPhim?.slice(0, 10).map((lichChieu, index) => {
                                                    return <NavLink to={`/checkout/${lichChieu.maLichChieu}`} key={index} className='ml-1 md:ml-[10px!important] text-[12px] sm:text-[14px] mt-2 text-sky-200 hover:text-sky-500 text-wrap break-all'>
